...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8639543648:AAGSL7OXPJylJkes32G4C7Dvne3oLScSSsc")
...
# =====================================================================
# USERBOT INSTANCE (CRASH‑PROOF VERSION)
# =====================================================================
class UserbotInstance:
    ...
    async def _chud_cmd(self, event):
        try:
            ...
            await self.edit_quote(event, "⚠️ An error occurred while enabling auto-reply.")
    ...
    async def _soja_cmd(self, event):
        try:
            ...
            await self.edit_quote(event, "⚠️ An error occurred while disabling auto-reply.")
    ...
    async def _resetchud_cmd(self, event):
        try:
            ...
            await self.edit_quote(event, "⚠️ An error occurred.")
    ...
    async def _creplyraid_cmd(self, event):
        try:
            ...
            await self.edit_quote(event, "⚠️ An error occurred while setting up custom reply raid.")
    ...
    async def _dcreplyraid_cmd(self, event):
        try:
            ...
            await self.edit_quote(event, "⚠️ An error occurred while deactivating custom reply raid.")
    ...
    async def _oneword_cmd(self, event):
        try:
            ...
            await self.edit_quote(event, "⚠️ An error occurred while starting oneword spam.")
...
class UserbotManager:
    ...
    async def restart_userbot(self, user_id: int):
        async with self._lock:
            ...
            # Rebuild the instance so /restart also works after an unexpected
            # disconnect or when the in-memory instance is missing.
